      ...
      {content.map((block, index) => {
        ...
        switch (block.type) {
          ...
          case 'header': {
            ...
            return (
              <HeaderTag key={index} id={id} className="group scroll-mt-32">
                ...
                <a
                  href={`#${id}`}
                  className="opacity-0 group-hover:opacity-100 transition-all ml-2 text-sm text-blue-500 font-bold hover:underline -mt-1 inline-block"
                >
                  #
                </a>
              </HeaderTag>
          ...
          case 'alert': {
            ...
            const colors = {
              warning: 'bg-yellow-50 border-l-[6px] border-yellow-400 text-yellow-800',
              tip: 'bg-emerald-50 border-l-[6px] border-emerald-400 text-emerald-800',
              info: 'bg-blue-50 border-l-[6px] border-blue-400 text-blue-800',
              error: 'bg-red-50 border-l-[6px] border-red-400 text-red-800',
            }
            ...
            return (
              <motion.div
                ...
                className={`p-8 rounded-3xl mb-12 shadow-lg ${colors[block.variant ?? 'info']}`}
              >
                    ...
                    <thead>
                      <tr className="bg-gradient-to-r from-slate-50 to-slate-100">
                        {block.headers?.map((header, i) => (
                          <th
                            key={i}
                            className="px-8 py-6 text-left font-black text-xl text-slate-900 border-b-2 border-slate-200"
                          >
                            {header}
                          </th>
                        ))}
